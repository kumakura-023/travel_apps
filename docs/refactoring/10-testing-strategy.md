...
describe('CreatePlanUseCase', () => {
  ...
  it('should throw error when user not found', async () => {
    ...
    // Act & Assert
    await expect(useCase.execute(command)).rejects.toThrow('ユーザーが見つかりません')
  })
  ...
  it('should throw error when plan limit exceeded', async () => {
    ...
    // Act & Assert
    await expect(useCase.execute(command)).rejects.toThrow('プランは最大50個まで作成できます')
  })
})
```
...
test.describe('Plan Management', () => {
  ...
  test('should create new plan', async ({ page }) => {
    ...
    // 作成されたプランが表示されることを確認
    await expect(page.locator('[data-testid=plan-name]')).toHaveText('E2Eテストプラン')
  })
  ...
  test('should add place to plan', async ({ page }) => {
    ...
    await page.selectOption('[data-testid=place-category-select]', 'restaurant')
