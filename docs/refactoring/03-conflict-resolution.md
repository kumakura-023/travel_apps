...
export class ConflictResolver {
  ...
  private resolveWithLastWriteWins(
    ...
    // 最後の変更時刻で判定
    return remote.lastChange.timestamp > local.lastChange.timestamp ? remote : local
  }
...
export class ConflictCoordinator {
  ...
  handleRemoteChange(local: VersionedPlan, remote: VersionedPlan): {
    ...
    // 自己変更かチェック
    if (this.detector.isSelfChange(remote.lastChange, this.sessionManager.sessionId)) {
      return {
        resolved: local, // 変更なし
        hadConflicts: false,
...
export function useConflictResolution(userId: string) {
  ...
  const handleRemoteChange = useCallback((local: TravelPlan, remote: TravelPlan) => {
    if (!coordinatorRef.current) return { resolved: remote, hadConflicts: false }
