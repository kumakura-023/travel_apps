...
export default function SelectionBanner() {
  ...
  const selectedPlace = places.find(
    (p) => p.id === selectionState.selectedPlaceId,
  );
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 mx-4 mt-4">
      <div
        className="glass-effect rounded-xl shadow-elevation-3 p-4 
      ...
      >
        <div className="flex items-center space-x-3 flex-1">
          <div
            className="flex-shrink-0 w-10 h-10 bg-coral-500/20 rounded-full 
          ...
          <div className="flex-1 min-w-0">
            <h3 className="headline text-system-label mb-1">ルート作成中</h3>
            <p className="subheadline text-system-secondary-label">
              <span className="font-semibold text-coral-600">{placeName}</span>{" "}
              から
            </p>
