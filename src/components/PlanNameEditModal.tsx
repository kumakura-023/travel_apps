...
const PlanNameEditModal: React.FC<PlanNameEditModalProps> = ({
  isOpen,
  onClose,
  isNewPlanCreation = false,
}) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {activeTab === "edit" ? (
          plan ? (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <label className="subheadline block text-system-label">
                  プラン名
                </label>
                <input
                  type="text"
              ...
              <div className="bg-coral-500/5 rounded-lg p-3 border border-coral-500/10">
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-system-secondary-label">
                      最終更新:
                    </span>
                    <span className="text-system-label font-medium">
            ...
            <div className="text-center py-8">
              <p className="text-system-secondary-label">
                プランが選択されていません
              </p>
              <p className="text-system-tertiary-label text-sm mt-2">
                管理タブから既存のプランを選択するか、新しいプランを作成してください
              </p>
            </div>
          )
        ) : (
          <div className="space-y-4">
            ...
            {/* 保存済みプラン */}
            <div>
              <h3 className="subheadline text-system-label mb-3">
                ...
                {savedPlans.length === 0 ? (
                  ...
                  savedPlans.map((savedPlan) => (
                    <div
                      ...
                      onClick={() =>
                        !isDeleteMode
                          ? handlePlanSelect(savedPlan)
                          : togglePlanSelection(savedPlan.id)
                      }
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="caption-1 text-system-secondary-label mt-1">
                            {"places" in savedPlan
                              ...
                              : `${(savedPlan as PlanListItem).placeCount}地点 • ¥${(savedPlan as PlanListItem).totalCost.toLocaleString()}`}
                          </p>
                        </div>
              ...
              if (isNewPlan && name.trim() === "" && plan) {
                if (
                  ...
                  try {
                    ...
                    console.error(
                      "[PlanNameEditModal] Failed to delete empty plan:",
                      error,
                    );
                  }
                }
      ...
      {showBulkDeleteConfirm && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-md z-[1002] flex items-center justify-center p-4
                       animate-modal-fade-in"
        >
          <div
            className="glass-effect rounded-2xl w-full max-w-sm p-6 text-center space-y-5
                         shadow-[0_32px_64px_0_rgba(0,0,0,0.4)]
                         animate-modal-zoom-in"
          >
