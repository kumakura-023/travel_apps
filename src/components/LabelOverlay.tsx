...
export default function LabelOverlay({
    ...
    () =>
      label.linkedPlaceId
        ? useSavedPlacesStore
            .getState()
            .getFilteredPlaces()
            .find((p) => p.id === label.linkedPlaceId)
        : null,
    [label.linkedPlaceId],
  );
  ...
  if (
    !label.position ||
    typeof label.position.lat !== "number" ||
    typeof label.position.lng !== "number"
  ) {
      ...
      >
        ...
        <div className="flex items-center gap-1 px-3 py-2 overflow-hidden">
          ...
          <span
            ...
            title={
              linkedPlace
                ? `${linkedPlace.name}にリンク: ${label.text}`
                : label.text
            }
          >
            {label.text || "メモ"}
