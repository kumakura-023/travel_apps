...
const InviteUrlModal: React.FC<InviteUrlModalProps> = ({
  isOpen,
  onClose,
  planId,
}) => {
  ...
  const handleGenerate = async () => {
    ...
    setCopied(false);
    try {
      const generateInviteToken = httpsCallable(
        functions,
        "generateInviteToken",
      );
  ...
  return (
    <ModalPortal>
      <div
        className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[1000] flex justify-center items-center p-4 animate-modal-fade-in"
        onClick={onClose}
      >
        <div
          className="glass-effect rounded-xl w-auto max-w-md min-w-[280px] mx-auto p-6 md:p-8 space-y-6 shadow-elevation-5 animate-modal-zoom-in flex flex-col"
        ...
        >
          ...
          <div className="space-y-4 text-center">
            ...
            {loading ? (
              <div className="text-center text-system-secondary-label">
                生成中...
              </div>
            ) : inviteUrl ? (
