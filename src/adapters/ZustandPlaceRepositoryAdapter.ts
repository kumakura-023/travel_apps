...
import {
  PlaceRepository,
  PlaceChangeEvent,
  PlaceRepositoryObserver,
} from "../interfaces/PlaceRepository";
 ...
 */
export class ZustandPlaceRepositoryAdapter
  implements PlaceRepository, PlaceRepositoryObserver
{
  ...
  add(place: Omit<Place, "id" | "createdAt" | "updatedAt">): Place {
    ...
    // 追加されたプレースを取得
    const addedPlace = this.getAll().find(
      (p) =>
