...
export function useConflictResolution(userId: string | null) {
  ...
  useEffect(() => {
    ...
    console.log(
      "[useConflictResolution] Conflict resolution system initialized for user:",
      userId,
    );
  }, [userId]);
  ...
  const handleRemoteChange = useCallback(
    (
      ...
      try {
        ...
        const versionedLocal =
          coordinatorRef.current.convertToVersionedPlan(local);
        const versionedRemote =
          coordinatorRef.current.convertToVersionedPlan(remote);

        // 競合解決を実行
        const result = coordinatorRef.current.handleRemoteChange(
          versionedLocal,
          versionedRemote,
        );

        // 結果をTravelPlan形式に戻す
        const resolvedTravelPlan = coordinatorRef.current.convertToTravelPlan(
          result.resolved,
        );
        ...
        console.error(
          "[useConflictResolution] Error handling remote change:",
          error,
        );
        return null;
      }
