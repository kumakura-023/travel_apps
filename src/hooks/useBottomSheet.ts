...
function bottomSheetReducer(
  ...
  switch (action.type) {
    ...
    case "END_DRAG": {
      ...
      const targetPercent = getNextSnapPoint(
        action.percent,
        action.snapPoints,
        direction,
      );
...
export function useBottomSheet(
  ...
  const minSnap = (
    isStandalone ? [10, 55, 80] : DEFAULT_SNAP_POINTS_BROWSER
  )[0];
  ...
  const prevStateRef = useRef<{ percent: number; isDragging: boolean }>({
    percent: initialPercent,
    isDragging: false,
  });
  useEffect(() => {
    ...
    if (
      currentState.percent !== prevState.percent ||
      currentState.isDragging !== prevState.isDragging
    ) {
  ...
  const bindHandleRef = useCallback(
    ...
    [supportsPointer, handlePointerDown, handleTouchStart],
  );
