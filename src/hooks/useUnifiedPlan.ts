...
export function useUnifiedPlan(userId: string | null) {
  ...
  const executeOperation = useCallback(async (
    operation: () => Promise<PlanOperationResult>
  ): Promise<boolean> => {
    if (!userId) {
  ...
  const createPlan = useCallback((name: string) => {
    return executeOperation(() => planServiceRef.current.createPlan(userId!, name));
  }, [userId, executeOperation]);

  const switchPlan = useCallback((planId: string) => {
    return executeOperation(() => planServiceRef.current.switchPlan(planId, userId!));
  }, [userId, executeOperation]);

  const duplicatePlan = useCallback((planId: string) => {
    return executeOperation(() => planServiceRef.current.duplicatePlan(userId!, planId));
  }, [userId, executeOperation]);

  const deletePlan = useCallback((planId: string) => {
    return executeOperation(() => planServiceRef.current.deletePlan(userId!, planId));
  }, [userId, executeOperation]);

  const updatePlanName = useCallback((planId: string, newName: string) => {
    return executeOperation(() => planServiceRef.current.updatePlanName(userId!, planId, newName));
  }, [userId, executeOperation]);
