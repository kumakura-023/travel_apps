...
export function useAutoSave(
  ...
  const [isRemoteUpdateInProgress, setIsRemoteUpdateInProgress] =
    useState(false);
  ...
  const calculatePlanHash = useCallback((plan: TravelPlan): string => {
    ...
    const placeIds = plan.places
      .map((p) => p.id)
      .sort()
      .join(",");
    const labelIds = plan.labels
      .map((l) => l.id)
      .sort()
      .join(",");
  ...
  // 新しい同期システム経由の保存関数
  const saveWithSyncManager = useCallback(
    async (
      plan: TravelPlan,
      operationType:
        | "place_added"
        ...
        | "label_added"
        | "label_updated"
        | "label_deleted" = "place_updated",
    ) => {
      const context = getSyncContext();
      await syncManagerRef.current.queueOperationLegacy(
        operationType,
        plan,
        context,
      );
  ...
  // 即座クラウド同期関数（イベントベース）
  const saveImmediatelyCloud = useCallback(
  ...
  const batchCloudSync = useCallback(
    async (plan: TravelPlan) => {
      if (!navigator.onLine || !user) return;
  ...
  useEffect(() => {
    ...
    if (
      lastWriteCompletedRef.current > 0 &&
      now - lastWriteCompletedRef.current < 500
    ) {
      return;
    }
