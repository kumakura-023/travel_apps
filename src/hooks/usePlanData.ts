...
import {
  ...
  PlaceViewModel,
  LabelViewModel,
} from "../domain/models/PlanViewModel";
...
export function usePlanData(): PlanDataHook {
  ...
  const plan: PlanViewModel | null = useMemo(() => {
    ...
    const placesWithSelection: PlaceViewModel[] = planWithData.places.map(
      (p) => ({
        id: p.id,
        name: p.name,
    ...
    const labelsWithSelection: LabelViewModel[] = planWithData.labels.map(
      (l) => ({
        id: l.id,
        text: l.text || "",
        position: l.position,
  ...
  const updatePlanName = useCallback(
    async (name: string) => {
      ...
      const unifiedPlanService =
        planCoordinatorRef.current.getUnifiedPlanService();
  ...
  const updatePlace = useCallback(
    async (id: string, update: Partial<Place>) => {
      if (!plan) return;
  ...
  const updateLabel = useCallback(
    async (id: string, update: Partial<Label>) => {
      if (!plan) return;
