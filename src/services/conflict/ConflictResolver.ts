...
export class ConflictResolver {
  ...
  resolve(
    ...
    console.log(
      `[ConflictResolver] Resolving ${conflicts.length} conflicts with strategy: ${this.strategy}`,
    );
  ...
  private resolveWithLastWriteWins(
    ...
    const shouldUseRemote =
      remote.lastChange.timestamp > local.lastChange.timestamp;
    const winner = shouldUseRemote ? remote : local;

    console.log(
      `[ConflictResolver] Last-write-wins: Using ${shouldUseRemote ? "remote" : "local"} (${winner.lastChange.timestamp})`,
    );
  ...
  private resolveWithUserChoice(
    ...
    console.log(
      `[ConflictResolver] Auto-resolving user choices based on severity...`,
    );
  ...
  private resolveMetadataConflict(
    resolved: VersionedPlan,
    conflict: ConflictInfo,
    remote: VersionedPlan,
  ): void {
    ...
    if (conflict.field === "name" || conflict.field === "description") {
      ...
      if (
        String(conflict.remoteValue || "").length >
        String(conflict.localValue || "").length
      ) {
  ...
  private resolvePlaceConflict(
    ...
    if (conflict.field === "existence") {
      ...
      if (conflict.remoteValue && !conflict.localValue) {
        ...
        if (resolved.places) {
          resolved.places = resolved.places.filter(
            (p) => p.id !== conflict.itemId,
          );
        }
      }
    } else {
  ...
  private resolveLabelConflict(
    ...
    if (conflict.field === "existence") {
      ...
      if (conflict.remoteValue && !conflict.localValue) {
        ...
        if (resolved.labels) {
          resolved.labels = resolved.labels.filter(
            (l) => l.id !== conflict.itemId,
          );
        }
      }
    } else {
  ...
  private updatePlaceField(
    resolved: VersionedPlan,
    conflict: ConflictInfo,
  ): void {
    if (!resolved.places) return;

    const placeIndex = resolved.places.findIndex(
      (p) => p.id === conflict.itemId,
    );
    if (placeIndex >= 0) {
      // より新しい変更を採用
      if (conflict.remoteChange.timestamp > conflict.localChange.timestamp) {
        (resolved.places[placeIndex] as any)[conflict.field] =
          conflict.remoteValue;
      }
    }
  }

  private updateLabelField(
    resolved: VersionedPlan,
    conflict: ConflictInfo,
  ): void {
    if (!resolved.labels) return;

    const labelIndex = resolved.labels.findIndex(
      (l) => l.id === conflict.itemId,
    );
    if (labelIndex >= 0) {
      // より新しい変更を採用
      if (conflict.remoteChange.timestamp > conflict.localChange.timestamp) {
        (resolved.labels[labelIndex] as any)[conflict.field] =
          conflict.remoteValue;
      }
    }
  }
  ...
  private mergeNewItems(
    ...
    const newRemotePlaces = (remote.places || []).filter(
      (p) => !localPlaceIds.has(p.id),
    );
    ...
    if (newRemotePlaces.length > 0) {
      ...
      console.log(
        `[ConflictResolver] Added ${newRemotePlaces.length} new places from remote`,
      );
    }

    // リモートの新しいラベルを追加
    const localLabelIds = new Set((local.labels || []).map((l) => l.id));
    const newRemoteLabels = (remote.labels || []).filter(
      (l) => !localLabelIds.has(l.id),
    );
    ...
    if (newRemoteLabels.length > 0) {
      ...
      console.log(
        `[ConflictResolver] Added ${newRemoteLabels.length} new labels from remote`,
      );
    }
  }
    ...
    return {
      ...
      changeHistory: [
        ...
        ...(remote.changeHistory || []).filter(
          (h) =>
            !(local.changeHistory || []).some(
              (lh) => lh.changeId === h.changeId,
            ),
        ),
        newChange,
      ],
