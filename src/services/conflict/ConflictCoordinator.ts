...
import { VersionedPlan, ConflictResolutionResult, ResolutionLog, ChangeMetadata } from '../../types/ConflictResolution';
...
export class ConflictCoordinator {
  ...
  handleRemoteChange(local: VersionedPlan, remote: VersionedPlan): ConflictResolutionResult {
    console.log(`[ConflictCoordinator] Handling remote change: local v${local.version}, remote v${remote.version}`);
    
    // 自己変更かチェック
    if (this.detector.isSelfChange(remote.lastChange, this.sessionManager.getCurrentSessionId())) {
      return {
        resolved: local, // 変更なし
        hadConflicts: false,
        resolutionLog: { 
    ...
    if (conflicts.length === 0) {
      console.log(`[ConflictCoordinator] No conflicts detected, accepting remote changes`);
      return {
    ...
    // 競合解決
    console.log(`[ConflictCoordinator] Resolving ${conflicts.length} conflicts...`);
    const resolved = this.resolver.resolve(conflicts, local, remote);
    
    // 競合の詳細をログに記録
    const conflictItems = conflicts.map(c => `${c.type}:${c.itemId}:${c.field}`);
  ...
  convertToVersionedPlan(plan: TravelPlan, sessionId?: string): VersionedPlan {
    const currentSessionId = sessionId || this.sessionManager.getCurrentSessionId();
   ...
   * 解決戦略を変更
   */
  setResolutionStrategy(strategy: 'last-write-wins' | 'merge-non-conflicting' | 'user-choice') {
