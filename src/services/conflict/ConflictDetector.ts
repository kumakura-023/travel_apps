...
export class ConflictDetector {
  ...
  private detectMetadataConflicts(local: VersionedPlan, remote: VersionedPlan): ConflictInfo[] {
    const conflicts: ConflictInfo[] = [];
  ...
  private detectPlaceConflicts(localPlaces: Place[], remotePlaces: Place[]): ConflictInfo[] {
    const conflicts: ConflictInfo[] = [];
  ...
  private comparePlaces(local: Place, remote: Place): ConflictInfo[] {
    const conflicts: ConflictInfo[] = [];
    const fields = ['name', 'address', 'category', 'memo', 'estimatedCost', 'scheduledDay'];
    ...
    // 座標の競合チェック（より精密に）
    if (Math.abs(local.coordinates.lat - remote.coordinates.lat) > 0.000001 ||
  ...
  private detectLabelConflicts(localLabels: MapLabel[], remoteLabels: MapLabel[]): ConflictInfo[] {
    const conflicts: ConflictInfo[] = [];
  ...
  private compareLabels(local: MapLabel, remote: MapLabel): ConflictInfo[] {
    const conflicts: ConflictInfo[] = [];
    const fields = ['text', 'fontSize', 'fontFamily', 'color', 'width', 'height', 'linkedPlaceId'];
    ...
    // 位置の競合チェック
    if (Math.abs(local.position.lat - remote.position.lat) > 0.000001 ||
  ...
  private getFieldSeverity(type: 'place' | 'label', field: string): 'low' | 'medium' | 'high' {
  ...
  private createDummyChange(timestamp: Date, operationType: 'add' | 'update' | 'delete'): ChangeMetadata {
    return {
