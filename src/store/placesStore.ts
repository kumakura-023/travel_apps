    ...
    if (event.type === "PLACE_ADDED") {
      set((state) => {
        ...
        const updatedPlaces = new Map(
          planPlaces.set(event.place.id, event.place),
        );
        const updatedPlans = new Map(
          state.placesByPlan.set(event.planId, updatedPlaces),
        );
    ...
    if (event.type === "PLACE_UPDATED") {
      set((state) => {
        ...
        if (planPlaces && planPlaces.has(event.placeId)) {
          ...
          const updatedPlaces = new Map(
            planPlaces.set(event.placeId, updatedPlace),
          );
          const updatedPlans = new Map(
            state.placesByPlan.set(event.planId, updatedPlaces),
          );
  ...
  return {
    ...
    _addPlace: (planId: string, place: Place) => {
      set((state) => {
        ...
        const updatedPlans = new Map(
          state.placesByPlan.set(planId, updatedPlaces),
        );
