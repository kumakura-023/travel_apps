...
export const useLabelsStore = create<LabelsState>((set, get) => {
  ...
  const unsubscribeLabelAdded = storeEventBus.on('LABEL_ADDED', (event) => {
    if (event.type === 'LABEL_ADDED') {
      set((state) => {
        ...
        const updatedLabels = new Map(planLabels.set(event.label.id, event.label));
        const updatedPlans = new Map(state.labelsByPlan.set(event.planId, updatedLabels));
    ...
    if (event.type === 'LABEL_UPDATED') {
      set((state) => {
        ...
        if (planLabels && planLabels.has(event.labelId)) {
          ...
          const updatedLabels = new Map(planLabels.set(event.labelId, updatedLabel));
          const updatedPlans = new Map(state.labelsByPlan.set(event.planId, updatedLabels));
          
          return { 
            labelsByPlan: updatedPlans,
            labels: state.labels.map(l => l.id === event.labelId ? { ...l, ...event.changes } : l) // 互換性のため
          };
        }
        return state;
      });
    ...
    if (event.type === 'LABEL_DELETED') {
      set((state) => {
        ...
        if (planLabels && planLabels.has(event.labelId)) {
          ...
          const newSelectedLabelId = state.selectedLabelId === event.labelId ? null : state.selectedLabelId;
  ...
  return {
    ...
    updateLabel: (id, update, localOnly = false) => {
      console.warn('[labelsStore] updateLabel is deprecated. Use events instead.');
      set((s) => {
    ...
    deleteLabel: (id) => {
      console.warn('[labelsStore] deleteLabel is deprecated. Use events instead.');
    ...
    clearLabels: () => {
      console.warn('[labelsStore] clearLabels is deprecated. Use events instead.');
      set({ labels: [] });
    }
  };
}); 