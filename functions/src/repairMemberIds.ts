 ...
 * 一度だけ実行すればよい
 */
export const repairExistingPlansMemberIds = functions.https.onCall(async (data, context) => {
 ...
 */
export const repairSinglePlanMemberIds = functions.https.onCall(async (data, context) => {
  const { planId } = data;
  ...
  if (needsUpdate) {
    ...
    console.log(`Successfully repaired plan ${planId}: memberIds updated to ${actualMemberIds}`);
