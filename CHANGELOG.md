...
### 🐛 バグ修正
- **スマホ版のメモ操作性の改善**: スマートフォンでメモ（ラベル）を移動またはリサイズする際に、背景の地図が一緒に動いてしまう問題を修正しました。
  - メモのドラッグ中およびリサイズ中も地図操作を無効化するようにし、意図しないマップのパンやズームを防ぎます。
  - これにより、ユーザーはメモの編集操作に集中できるようになり、操作性が向上します。
