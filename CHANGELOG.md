...
### 🐛 バグ修正
- **クラウド同期**: プラン名や旅程（開始日・終了日）を変更しても、その内容がクラウドに保存されていなかった問題を修正しました。
  - **原因**: 自動保存の変更検知ロジックが、プラン名や日付の変更を監視していませんでした。
  - **修正内容**: 変更検知のためのハッシュ計算にプラン名と日付を含めるように修正しました。これにより、これらの情報が変更された際にも自動保存が正しく実行され、クラウドと同期されるようになります。
