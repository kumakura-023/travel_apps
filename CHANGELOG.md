...
- **共有・招待UIをアカウントアイコンのメニューに統合**
  - 画面右上のアカウントアイコンをクリックすると、「ログイン/ログアウト」「ほかのアカウントを招待」「URLで招待」の各機能にアクセスできるようになりました。
  - これにより、共有・招待機能がどの画面幅・デバイスでも確実に利用できるようになりました。
- **「URLで招待」ボタンの動作修正**
  - 「URLで招待」ボタンをクリックすると、必ず招待URL生成ダイアログが開くようになりました。

### 新機能

- **URL招待機能の新規実装**
  - プランごとに一意な招待URLを発行し、URLを知っているユーザーがアクセスすることで共同編集メンバーとして参加できるようになりました。
  - 招待URLはオーナー/編集者のみが発行可能で、セキュリティ上推測困難なトークンを含みます。
  - /invite/:token ルートでアクセス時、未ログインなら認証後に自動でプラン参加処理が行われます。
- **招待機能UIの追加**
  - 「プランを共有」ダイアログに「URLで招待」ボタンを追加し、ワンクリックで招待URLを生成・コピーできるようになりました。
  - 招待URLを受け取ったユーザーは、アクセスするだけで簡単にプランに参加できます。
...
- **招待ホストとゲスト間の編集内容共有問題を修正**
  - **問題**: ゲスト側で招待以前に編集していた内容が表示され、プランに参加しても参加したプランの内容にアクセスできない状態でした。
  - **原因**: 招待参加後にプランストアの状態が古いまま残り、新しいプランの読み込みが正しく行われていませんでした。
  - **修正内容**:
    1. `InviteAcceptPage.tsx`で招待参加時にプランストアの状態を完全にクリアするように修正
    2. `planCloudService.ts`に`loadPlanById`関数を追加し、指定されたプランIDのプランを直接読み込めるように改善
    3. 招待参加後に新しいプランを直接読み込んでストアに設定し、リアルタイム同期リスナーが自動的に再設定されるように修正
  - **効果**: 招待参加後、ゲスト側は参加したプランの内容に正しくアクセスでき、ホストとゲスト間でリアルタイムに編集内容が共有されるようになりました。
  - Firestoreのplansコレクションで、membersフィールドにユーザー自身のUIDが存在しない場合でも、ownerIdが一致すればプランを取得できるように修正
  - さらに、members[uid]が存在しない場合は自動で追加することで、既存データも正常に読み込めるよう対応
...
- **共同編集機能の実装**: 複数のユーザーがリアルタイムで同じ旅行プランを編集できるようになりました。
  - **リアルタイム同期**: Firestoreの`onSnapshot`を利用し、あるユーザーの変更が即座に他の共同編集者の画面に反映されます。
  - **ユーザー招待**: メニューの「プランを共有」から、メールアドレスで他のユーザーをプランに招待できます。
  - **安全な招待プロセス**: ユーザー招待のロジックはFirebase Cloud Functionsで処理され、権限のないユーザーがメンバーを追加できないようにセキュリティが確保されています。

### 🔧 技術的改善

- **データモデルの拡張**: Firestoreのデータ構造を、プランごとに`members`フィールドを持つ形に変更し、共同編集に対応しました。
- **バックエンドロジック**: Firestoreのセキュリティルールを更新し、プランの所有者とメンバーのみがデータの読み書きを行えるように制限しました。
- **状態管理の強化**: Zustandの`planStore`を改修し、Firestoreのリアルタイムリスナー（`onSnapshot`）と連携して、プランの状態を同期するようにしました。
- **Cloud Functions導入**: Firebase Cloud Functions (v2) を導入し、メールアドレスからユーザーを検索してプランに招待する`inviteUserToPlan`関数を実装しました。
...
- **メモ編集ダイアログの操作性改善**: メモ編集ダイアログ内でプルダウンメニューを操作した際に、意図せずダイアログが閉じてしまう問題を修正しました。背景部分が直接クリックされた場合のみダイアログが閉じるようにイベント処理を改善し、安定した操作性を実現しました。
- **候補地オーバーレイの訪問日ピッカー修正**: 地図上の候補地オーバーレイで訪問日ピッカーをクリックした際に、すぐに閉じてしまう問題を修正しました。
  - **原因**: 地図のクリックイベントがオーバーレイのクリックイベントを上書きし、select要素のドロップダウンが正常に開かない状態でした。
  - **修正内容**:
    1. オーバーレイ全体に`onClick`と`onMouseDown`でイベント伝播制御（`e.stopPropagation()`）を追加
    2. 訪問日ピッカー（select要素）に`onClick`、`onMouseDown`、`onFocus`でのイベント伝播制御を強化
    3. 削除ボタン、出発地・目的地ボタンにも同様のイベント制御を追加
    4. `pointerEvents: 'auto'`を明示的に設定してポインターイベントを有効化
  - **効果**: 訪問日ピッカーが正常にドロップダウンを開き、地図のクリックイベントに干渉されることなく操作できるようになりました。

### 🎨 UI改善

- **訪問日ピッカーの視認性向上**: 候補地オーバーレイの訪問日ピッカーの文字色を黒色（`rgba(0, 0, 0, 0.85)`）に変更し、白い背景に対するコントラストを改善しました。これにより、選択肢がより読みやすくなりました。
...
- **カレンダーUIの不具合修正**: 年月ピッカーを開くための状態変数が定義されていなかったため、カレンダーを開こうとするとクラッシュする問題を修正しました。
...
- **プラン名・旅程の保存**: プラン名や旅程の編集後、「保存」ボタンを押したタイミングで即座にクラウド同期が実行されるように修正しました。これにより、自動保存のタイミングに依存することなく、ユーザーの操作に基づいて確実にデータが保存されるようになります。

## v1.4.48 (2025-07-16)

### 🐛 バグ修正

- **クラウド同期の信頼性向上**: プラン名・日付の変更がクラウドへ保存されない根本原因を修正しました。
  - **原因**: プラン名や日付が変更された際に、クラウド同期をトリガーするためのイベント通知が実装されていませんでした。
  - **修正内容**: `planStore`に`onPlanUpdated`コールバックを追加し、プラン情報が更新されるたびにイベントを発行するようにしました。`App.tsx`がこのイベントを監視し、クラウドへの保存処理を呼び出すことで、変更が確実かつ即座に同期されるようになります。
...
- **地図上のメモのドラッグ操作を修正**: PC版で、地図上のメモ（ラベル）にカーソルを合わせただけで意図せず移動が開始されてしまう問題を修正しました。
  - **原因**: マウスボタンが押されているかどうかの状態を管理していなかったため、ホバーするだけでドラッグが開始される場合がありました。
  - **修正内容**: マウスボタンが押されている間だけドラッグ処理が実行されるようにロジックを修正しました。これにより、クリックしてドラッグするという標準的な操作でメモが移動するようになります。
...
- **ビルドエラーの修正**: `LabelOverlay.tsx` に誤って挿入された不正な文字列リテラルを削除し、ビルドが正常に完了するように修正しました。
